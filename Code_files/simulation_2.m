% MATLAB Code for Generalized Richards Model (GRM) Data Generation

function generalized_richards_model()
    % Parameters
    r = 0.2;
    p = 0.8;
    K = 1000;
    ...
    data1=fig_data(t_window,:); % select the early growth phase of the epidemic
    ...
    line2=plot(fig_data(:,1)*DT,fig_data(:,2),'bo')
    set(line2,'LineWidth',2)
    ...
    line1=plot(fig_data(:,1)*DT,fig_data(:,2),'ko')
    set(line1,'LineWidth',3)
    
    % plot vertical line separating calibration versus forecast periods
    line2=[timevect(end) 0;timevect(end) max(fig_data(:,2))*2];
    ...
    save(strcat('Forecast-GGM-M-',num2str(M),'-tf-',num2str(t_window(end)),'-horizon-',num2str(forecastingperiod),'-flag-',num2str(flag1),'-dist-',num2str(dist1),'-factor-',num2str(factor1),'-w-',num2str(alpha1),'.mat'), '-mat')
    ...
    xlabel('Time (t)');
    ylabel('Population (N)');
    title('Synthetic Data Generation using Generalized Richards Model');
    grid on;
...
function dNdt = grm_function(N, r, p, K, nu)
    dNdt = r * (N^p) * (1 - (N / K)^nu);
end
