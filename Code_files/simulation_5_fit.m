...
datafilename1='1918_influenza.txt';
...
ylabel('Incidence','fontweight','bold','fontsize',20);
legend('Model','Data');

set(gca, 'FontSize', 24, 'LineWidth', 3, 'FontWeight', 'bold');
set(gcf,'color','white');
...
xlabel('Time (days)','fontweight','bold','fontsize',20);
ylabel('Residuals','fontweight','bold','fontsize',20);
...
for realization = 1:M
    ...
    LB = [0  0];   % lower bounds for r and p
    UB = [20 1];   % upper bounds for r and p
    ...
    % Fit model to bootstrap realization
    [P, resnorm, residual] = lsqcurvefit(@plotSimpleGrowth1, z, timevect, ...
        weights .* yirData, LB, UB, options, I0, flag1, weights);
    
    % Extract fitted parameters
    r_hat = P(1);
    p_hat = P(2);
    
    lambda = 1/4; % For mean = 4
    Rt_values = zeros(length(yirData), 1);
    Rt_values(1) = NaN; % Undefined at t=1
    
    for tj = 2:length(yirData)
        num = yirData(tj);
        
        denom = 0;
        for ti = 1:tj-1
            w = lambda * exp(-lambda * (tj - ti));
            denom = denom + yirData(ti) * w;
        end
        
        if denom > 0
            Rt_values(tj) = num / denom;
        else
            Rt_values(tj) = NaN;
        end
    end

    % Store mean Reff across the valid time points
    R_eff_vals(realization) = mean(Rt_values(~isnan(Rt_values)));
    
    % Store parameter estimates and Reff
...
param_reff=[mean(R_eff_vals) plims(R_eff_vals,0.025) plims(R_eff_vals,0.975)];
...
cad3=strcat('Reff=',num2str(param_reff(end,1),2),' (95% CI:',num2str(param_reff(end,2),2),',',num2str(param_reff(end,3),2),')')
...
subplot(2,3,3)
hist(R_eff_vals)
xlabel('Reff','fontweight','bold','fontsize',20);
ylabel('Frequency','fontweight','bold','fontsize',20)
...
% forecast horizon
forecastingperiod=10; % number of data points ahead
